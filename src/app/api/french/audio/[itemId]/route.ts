// On-demand French TTS for the Compréhension de l'oral (Listening) skill.
...
export async function GET(
  ...
  const item = await prisma.frenchItem.findFirst({
    where: { id: itemId, active: true, skill: "COMPREHENSION_ORALE" },
  });
  if (!item) {
  ...
  try {
    ...
    console.error("[french.audio] tts failed:", err);
