...
// Posts to /api/french/submit and renders the engine estimate.

import { useState } from "react";
import type { FrenchItem } from "@prisma/client";
...
export type ComposerItem = Pick<FrenchItem, "id" | "skill" | "title" | "prompt"> & { payload: Payload };
...
export function Composer({ item, examId }: { item: ComposerItem; examId: string }) {
  ...
  const isReading = item.skill === "COMPREHENSION_ECRITE";
  const isListening = item.skill === "COMPREHENSION_ORALE";
  const isWriting = item.skill === "EXPRESSION_ECRITE";
  const isSpeaking = item.skill === "EXPRESSION_ORALE";
  ...
  return (
    <div className="mt-4">
      ...
      {isListening && (
        <audio controls preload="none" className="mt-4 w-full" src={`/api/french/audio/${item.id}`}>
      ...
      {(isWriting || isSpeaking) && (
        <div className="mt-5">
          ...
          <textarea
            ...
            placeholder={isWriting ? "Votre réponse en français…" : "Transcription de votre réponse…"}
