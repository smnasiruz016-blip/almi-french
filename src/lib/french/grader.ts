// AI grader for the productive skills (Expression écrite / Expression orale).
// Grades the candidate's FRENCH text (or Whisper transcript) against the official
// criteria FOR THE CHOSEN EXAM + LEVEL — we do NOT hardcode a fixed criteria
// count (it varies by exam/level). Returns an honest quality fraction (0–1, fed
// to the scoring bridge) plus qualitative feedback in English. Speaking is graded
// from the transcript only — never the accent. No fabricated official score.
...
export async function gradeProductive(args: {
  exam: Exam;
  level: CefrLevel;
  skill: FrenchSkill; // EXPRESSION_ECRITE | EXPRESSION_ORALE
  taskPrompt: string;
  response: string; // French text or transcript
  userId: string | null;
}): Promise<Grade> {
  const { exam, level, skill, taskPrompt, response, userId } = args;
  const isSpeaking = skill === "EXPRESSION_ORALE";
  ...
  const system = [
    ...
    isSpeaking
      ? "This is a transcript of spoken French. Judge language only (task achievement, coherence, range, accuracy). NEVER judge accent or pronunciation — they are not in the transcript."
      : "Judge task achievement, coherence/cohesion, lexical range, and grammatical accuracy as the official criteria define them for this exam and level.",
  ...
  try {
    ...
    await recordCost({
      userId,
      feature: isSpeaking ? "expression_orale.grade" : "expression_ecrite.grade",
      model: MODELS.SONNET,
      usage,
      success: true,
    });
